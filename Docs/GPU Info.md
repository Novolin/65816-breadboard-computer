...
I am also pulling an 8MHz clock signal from my clock circuit to provide a faster clock speed for both communications as well as DMA response.
...
VRAM is directly accessed by the CPU, using a DMA signal generated by accessing the memory location 0x<TBD>. This triggers the Z80's DMA behaviour, allowing the CPU to directly access VRAM. The delay between /BUSREQ and /BUSACK should only be 1 GPU clock cycle, meaning it will be over with before the CPU has finished getting the next opcode ready. 

I have yet to actually *do* this, but it would likely cause tearing/hitching if VRAM was changed during the frame drawing process. I think the SIO would continue to output correctly, and the CTC would operate as long as the Graphics clock isn't inhibited