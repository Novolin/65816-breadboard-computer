...
.include        "delay.s"
.ENDIF
.IFNDEF IOMEM ; Define IOMEM before include if you want a different address.
.FARADDR IOMEM = $010000
.ENDIF
...
; GFX LCD Definitions:
GLCD_RESET      = $10000000             ; Probably won't ever need to set this.
GLCD_CS         = $10111111             ; AND with whatever else, to bring CS low!
GLCD_DATA       = $00100000             ; Data = 1, Command = 0 

; I'm not going to write all of the commands because it's a goddamn book!! 
VIA_START:      ; set up the VIA to function for both displays.
 PHA                                    ; We're using the accumulator, so store its value
 LDA            #$FF                    ; Set up our DDRs, both all outputs
 STA            LCD::DDRB               
 STA            LCD::DDRA             
 LDA            #0                      ; Make sure the ports are empty
 STA            LCD::CONTROL
 STA            LCD::CHRDAT
 LDA            #%00010100              ; SR Out under T2 Control
 STA            LCD::ACR                
 LDA            #%01111011              ; Make sure all interrupts are disabled, except SR
 STA            LCD::IER                
 LDA            #%10000100              ; Make sure SR interrupt is actually enabled!
 STA            LCD::IER                
 LDA            #2                      ; T2 counter. TODO: make sure it's happy with the LCD.
 STA            LCD::T2CL
 PLA
 RTS            ; I think that's everything?
...
LCD_BOOT:       ; Enable the VIA, initialize the LCD to 'default' mode
 ...
 INC            LCD::CONTROL            ; Flag Enable bit
 STA            LCD::CHRDAT             ; Get the data packet out
 DEC            LCD::CONTROL            ; Latch it.
 JSR            LCD_WAIT                ; Loops until LCD is ready
 INC            LCD::CONTROL            ; LCD_WAIT will put the command port back to 0, so we just need the "E" bit to toggle.
 ...
 DEC            LCD::CONTROL            ; Latch it.
 PLA                                    ; Get accumulator back.
 RTS                                    ; And go back to what we were doing before!
 
LCD_SENDCHAR:  ; Send the character in "A" to the character LCD
 JSR            LCD_WAIT
 PHY                                    ; We're using Y to save our control flags
 PHA                                    ; Save the character for a moment
 LDA            LCD::CONTROL            
 TAY
 ORA            #LCD_CHAR               ; OR with our new flag
 STA            LCD::CONTROL            
 INC            LCD::CONTROL            ; Bring up E after a couple cycles, for stability's sake
 PLA                                    ; Ready for our character
 STA            LCD::CHRDAT             ; So give'r
 STY            LCD::CONTROL            ; Drop the enable bit, and return the flags to what they were before.
...
LCD_WAIT:      ; Wait for the LCD busy flag to clear
 ...
 TAY                                    ; Copy our control register to Y
 ORA            #LCD_RW                 ; Add the RW bit
 STA            LCD::CONTROL
 LDA            #0                      ; Set our DDR to in on the data lines.
 STA            LCD::DDRB             
@READBUSY
 INC            LCD::CONTROL            ; Fire the Enable bit
 ...
 STA            LCD::DDRB
 LDA            #0
 STY            LCD::CONTROL            ; Bring the Control port back to normal
 PLY                                    ; Get back our stuff.
